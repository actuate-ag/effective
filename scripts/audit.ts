#!/usr/bin/env bun
/**
 * effect-audit CLI: runs the claude-code-effect pattern catalog against
 ...
 *  - Skips symlinks (so a symlinked .references/effect-v4 doesn't pull in
 *    the entire Effect source).
...
const handler = Effect.fn('effect-audit')(
	function*(input: {
...
const audit = Command.make(
	...
	Command.withDescription(
		'Run the claude-code-effect pattern catalog against a file or directory tree.',
	),
);
