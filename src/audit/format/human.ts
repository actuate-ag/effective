...
const SEVERITIES: ReadonlyArray<Severity> = ["critical", "high", "medium", "warning", "info"];
...
const countSeverity = (matches: ReadonlyArray<AuditMatch>, severity: Severity): number =>
  Arr.filter(matches, (m) => m.severity === severity).length;
