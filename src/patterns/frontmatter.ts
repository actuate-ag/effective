...
const sanitize = (block: string): string => block.split("\n").map(quoteYamlValue).join("\n");
    ...
    Option.match({
      ...
      onSome: (block) =>
        Effect.try({
          ...
          catch: (cause) => new FrontmatterParseError({ path, message: String(cause) })
        }).pipe(
          Effect.map((parsed): Record<string, unknown> => {
            if (typeof parsed !== "object" || parsed === null) return {};
            const result: Record<string, unknown> = Object.fromEntries(Object.entries(parsed));
            return result;
          })
        )
    })
...
export const extractBody = (content: string): string => content.replace(/^---\n[\s\S]*?\n---\n?/, "").trim();
