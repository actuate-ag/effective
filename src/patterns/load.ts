...
const isAstRule = (v: unknown): v is AstGrepRuleDefinition => typeof v === "object" && v !== null && !Array.isArray(v);
...
const isAstRuleEntry = (entry: readonly [string, unknown]): entry is readonly [string, AstGrepRuleDefinition] =>
  isAstRule(entry[1]);
...
const astRuleRecord = (raw: unknown): Option.Option<Record<string, AstGrepRuleDefinition>> => {
  ...
  const valid = entries.filter(isAstRuleEntry);
  return valid.length === entries.length ? Option.some(Object.fromEntries(valid)) : Option.none();
};
...
const toSeverity = (raw: unknown): Severity =>
  pipe(
    ...
    Option.match({
      ...
      onSome: (s): Severity =>
        s === "critical" || s === "high" || s === "medium" || s === "warning" || s === "info" ? s : "info"
    })
  );
...
const toAstDetector = (raw: Record<string, unknown>): Option.Option<AstDetector> => {
  const ruleList = pipe(
    astRuleList(raw.rule),
    Option.orElse(() => astRuleList(raw.rules))
  );
...
const toRegexDetector = (raw: Record<string, unknown>): Effect.Effect<Option.Option<RegexDetector>> =>
  Effect.gen(function* () {
...
const readPatternFile = (filePath: string): Effect.Effect<Option.Option<Pattern>, never, FileSystem.FileSystem> =>
  Effect.gen(function* () {
    ...
    const raw = yield* parseFrontmatter(filePath, content.value).pipe(
      Effect.catchTag("FrontmatterParseError", () => Effect.succeed<Record<string, unknown>>({}))
    );
    return yield* toPattern(filePath, content.value, raw);
  });

const walk = (dir: string): Effect.Effect<ReadonlyArray<Pattern>, never, FileSystem.FileSystem | Path.Path> =>
  Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const emptyPatterns: ReadonlyArray<Pattern> = [];
    ...
    const results = yield* Effect.forEach(
      ...
      (entry) =>
        Effect.gen(function* () {
          ...
          if (Option.isNone(info)) return emptyPatterns;
          ...
          return Option.match(loaded, {
            onNone: (): ReadonlyArray<Pattern> => [],
            onSome: (p): ReadonlyArray<Pattern> => [p]
          });
        }),
      { concurrency: 8 }
    );
