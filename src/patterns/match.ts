...
const compileGlob = (g: string): Effect.Effect<Option.Option<(p: string) => boolean>> =>
  Effect.try({
    try: () => picomatch(g),
    catch: () => null
  }).pipe(
...
const langFromPath = (filePath: string): Option.Option<Lang> =>
  Match.value(filePath).pipe(
    Match.when(
      (p: string) => p.endsWith(".tsx"),
      () => Option.some(Lang.Tsx)
    ),
    Match.when(
      (p: string) => p.endsWith(".jsx"),
      () => Option.some(Lang.Tsx)
    ),
    Match.when(
      (p: string) => p.endsWith(".ts"),
...
const locationFromSpan = (source: string, start: number, end: number): MatchLocation => {
...
const regexLocations = (det: RegexDetector, source: string): Effect.Effect<ReadonlyArray<MatchLocation>> =>
  Effect.gen(function* () {
    const empty: ReadonlyArray<MatchLocation> = [];
    const reOpt = yield* compileRegex(det.pattern);
    if (Option.isNone(reOpt)) return empty;
...
const parseAst = (lang: Lang, source: string): Effect.Effect<Option.Option<AstRoot>> =>
  Effect.try({
...
const findAllSafe = (
  ...
  Effect.try({
    ...
    Effect.match({
      onFailure: (): ReadonlyArray<{ start: number; end: number }> => [],
      onSuccess: (xs) => xs
    })
  );
...
const astLocations = (
  ...
  Effect.gen(function* () {
    ...
    if (Option.isNone(lang)) return empty;

    const rootOpt = yield* parseAst(lang.value, source);
    if (Option.isNone(rootOpt)) return empty;
    ...
    const fromRules = yield* Effect.forEach(det.rules ?? [], (rule) => findAllSafe(root, astRuleMatcher(det, rule)), {
      concurrency: "unbounded"
    });
...
const passesToolFilter = (pattern: Pattern, toolName: string): Effect.Effect<boolean> =>
  pipe(
...
const passesGlobFilter = (pattern: Pattern, filePath: string): Effect.Effect<boolean> =>
  pipe(
...
const passesIgnoreGlobFilter = (pattern: Pattern, filePath: string): Effect.Effect<boolean> =>
  pipe(
...
export const findPatternMatches = (
  ...
  Effect.gen(function* () {
    const empty: ReadonlyArray<MatchLocation> = [];
    if (pattern.event !== "after") return empty;
